...
export default async function MulCottonPage() {
  ...
  const mulCottonProducts = (dbProducts || []).filter((p: any) => {
    const collectionSlug = p.collections?.slug?.toLowerCase() || ''
    const collectionName = p.collections?.name?.toLowerCase() || ''
    const productName = p.name?.toLowerCase() || ''
    
    return collectionSlug.includes('mul-cotton') || 
           collectionSlug.includes('mulcotton') ||
           collectionName.includes('mul cotton') || 
           collectionName.includes('mulcotton') ||
           productName.includes('mul cotton') ||
           productName.includes('mulcotton')
  })
      ...
      <section className="relative bg-primary px-6 py-16 lg:px-8 lg:py-24">
        <div className="mx-auto max-w-7xl">
          ...
          <p className="mt-4 max-w-2xl text-base leading-relaxed text-primary-foreground/80 lg:text-lg">
            Explore our lightweight and breathable Mul Cotton sarees, perfect for 
            everyday elegance and comfortable all-day wear.
          </p>
        </div>
      </section>
          ...
          {mulCottonProducts.length > 0 ? (
            <>
              ...
              <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                {mulCottonProducts.map((product: any) => (
                  <article
                    key={product.id}
            ...
            <div className="py-20 text-center">
              <p className="text-lg text-muted-foreground">
                No Mul Cotton sarees available at the moment. Check back soon!
              </p>
              <Link
                href="/"
