...
export default async function BaranasiPage() {
  ...
  const banarsiProducts = (dbProducts || []).filter((p: any) => {
    const collectionSlug = p.collections?.slug?.toLowerCase() || ''
    const collectionName = p.collections?.name?.toLowerCase() || ''
    const productName = p.name?.toLowerCase() || ''
    
    return collectionSlug.includes('banarasi') || 
           collectionName.includes('banarasi') || 
           productName.includes('banarasi')
  })
      ...
      <section className="relative bg-primary px-6 py-16 lg:px-8 lg:py-24">
        <div className="mx-auto max-w-7xl">
          ...
          <p className="mt-4 max-w-2xl text-base leading-relaxed text-primary-foreground/80 lg:text-lg">
            Discover our exquisite collection of Banarasi sarees featuring intricate 
            gold and silver zari work, woven by master craftsmen from Varanasi.
          </p>
        </div>
      </section>
          ...
          {banarsiProducts.length > 0 ? (
            <>
              ...
              <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                {banarsiProducts.map((product: any) => (
                  <article
                    key={product.id}
            ...
            <div className="py-20 text-center">
              <p className="text-lg text-muted-foreground">
                No Banarasi sarees available at the moment. Check back soon!
              </p>
              <Link
                href="/"
