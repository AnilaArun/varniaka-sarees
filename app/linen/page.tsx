...
export default async function LinenPage() {
  ...
  const linenProducts = (dbProducts || []).filter((p: any) => {
    const collectionSlug = p.collections?.slug?.toLowerCase() || ''
    const collectionName = p.collections?.name?.toLowerCase() || ''
    const productName = p.name?.toLowerCase() || ''
    
    return collectionSlug.includes('linen') || 
           collectionName.includes('linen') || 
           productName.includes('linen')
  })
      ...
      <section className="relative bg-primary px-6 py-16 lg:px-8 lg:py-24">
        <div className="mx-auto max-w-7xl">
          ...
          <p className="mt-4 max-w-2xl text-base leading-relaxed text-primary-foreground/80 lg:text-lg">
            Discover our premium linen sarees that combine comfort with contemporary 
            style, perfect for modern women who appreciate sustainable fashion.
          </p>
        </div>
      </section>
          ...
          {linenProducts.length > 0 ? (
            ...
            <div className="py-20 text-center">
              <p className="text-lg text-muted-foreground">
                No Linen sarees available at the moment. Check back soon!
              </p>
              <Link
                href="/"
